...
export type DispatcherFn<PAYLOAD_TYPE> = (dispatchFn: Dispatch, payload: PAYLOAD_TYPE) => ReturnType<Dispatch>;

export function dispatcher<PAYLOAD_TYPE>(actionName: string): DispatcherFn<PAYLOAD_TYPE> {
    return (dispatchFn: Dispatch, payload: PAYLOAD_TYPE) => dispatchFn(actionName, payload);
}
